...
**Path-resolution safeguard (always):** NEVER guess or hand-construct a source path. Every `read_file` target must originate from EITHER (1) a `graph.json` node's `source_file` field, OR (2) a `file_search` / `grep_search` result line. This workspace uses a double-nested root (`EverydayGoods/EverydayGoods/...` — outer = git root, inner = .csproj folder); hand-typing `EverydayGoods/Program.cs` fails, while the graph's `source_file` resolves correctly. If the nesting is ambiguous before the first read, `list_dir` the workspace root once to confirm.
