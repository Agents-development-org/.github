...
1a. **Graphify bootstrap (MUST — before project skill loading)** — resolve `{WORKSPACE_ROOT}` from the open workspace, never from an incidental terminal working directory, set the working directory to `{WORKSPACE_ROOT}`, then check `{WORKSPACE_ROOT}/graphify-out/graph.json` directly.
  ...
  - **Existing graph:** if `graph.json` exists, is readable, and is non-empty, set `GRAPHIFY-STATUS=READY` and continue without rebuilding it. If `graphify-out/graph.html` (the interactive graph view) is missing or empty, regenerate only the view — see Verification below — without rebuilding the graph.
  - **Missing or invalid graph:** invoke the host-installed Graphify skill exactly as `/graphify .` from `{WORKSPACE_ROOT}`. This is the only permitted setup invocation: do not add paths, modes, export options, or any other flags. The default `/graphify .` invocation emits the interactive graph view `graphify-out/graph.html` alongside `graph.json` — do NOT pass `--no-viz`, which would suppress the view.
  ...
  - **Verification and artifact:** after setup, verify BOTH `{WORKSPACE_ROOT}/graphify-out/graph.json` AND `{WORKSPACE_ROOT}/graphify-out/graph.html` (the interactive graph view) exist, are readable, and are non-empty. If `graph.json` verification fails, set `GRAPHIFY-STATUS=FAILED`, call `drax-coder/RecordPrompt` (`status="FAILED"`), report the setup error, and STOP before project-type detection or project skill loading. If `graph.json` is valid but `graph.html` is missing/empty, regenerate the view by re-invoking the host-installed Graphify skill exactly as `/graphify .` from `{WORKSPACE_ROOT}` (the default emits the HTML view) — do NOT pass `--no-viz`; do not rebuild the graph. On success set `GRAPHIFY-STATUS=CREATED` and record `GRAPHIFY-GRAPH=graphify-out/graph.json` AND `GRAPHIFY-VIEW=graphify-out/graph.html` as immutable workflow inputs.
...
flowchart TD
  ...
  GRAPH_CHECK -->|no| GRAPH_BUILD[Run installed Graphify skill]
  GRAPH_BUILD --> GRAPH_VERIFY{graph.json created and non-empty?}
  GRAPH_VERIFY -->|no| STOP_GRAPH[Record FAILED and STOP]
  GRAPH_VERIFY -->|yes| DETECT
  GRAPH_CHECK -->|yes| DETECT[Pre-flight 2a: detect PROJECT-TYPE from workspace .csproj files]
